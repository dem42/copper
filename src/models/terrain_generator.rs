...
impl HeightsGenerator {    
    ...
    // flat edge width -> used so that terrain cells link up correctly
    const EDGE_WIDTH: usize = 10;

    pub fn get_smooth_noise(&self, x: isize, y: isize, ow: usize) -> f32 {
        ...
        let corners = self.get_noise(x-1,y-1,ow) + self.get_noise(x-1,y+1,ow) + self.get_noise(x+1,y-1,ow) + self.get_noise(x+1,y+1,ow);
        let edges = 2.0 * (self.get_noise(x-1,y,ow) + self.get_noise(x+1,y,ow) + self.get_noise(x,y-1,ow) + self.get_noise(x,y+1,ow));
        let center = 4.0 * self.get_noise(x, y, ow);
        (center + edges + corners) / 16.0
    }    

    fn check_outside_autogenerated_bounds(&self, x: isize, y: isize, octave_width: usize) -> bool {
        x < (HeightsGenerator::EDGE_WIDTH/octave_width) as isize 
            || y < (HeightsGenerator::EDGE_WIDTH/octave_width) as isize
            || x > ((HeightsGenerator::WIDTH - HeightsGenerator::EDGE_WIDTH)/octave_width) as isize
            || y > ((HeightsGenerator::HEIGHT - HeightsGenerator::EDGE_WIDTH)/octave_width) as isize        
    }

    fn get_noise(&self, x: isize, y: isize, octave_width: usize) -> f32 {
        if self.check_outside_autogenerated_bounds(x, y, octave_width) {
            return 0.0;
        }
    ...
    // we pass in the octave width just to be able to clamp the region to 0 on the boundaries smoothly
    // this is needed for multiple terrain cells
    fn get_interpolated_noise(&self, x: f32, y: f32, octave_width: usize) -> f32 {
        let x_whole = x.floor();
        let y_whole = y.floor();
        ...
        let noise_1 = self.get_smooth_noise(x_whole, y_whole, octave_width);
        let noise_2 = self.get_smooth_noise(x_whole+1, y_whole, octave_width);
        let noise_3 = self.get_smooth_noise(x_whole, y_whole+1, octave_width);
        let noise_4 = self.get_smooth_noise(x_whole+1, y_whole+1, octave_width);
...
impl TerrainGenerator for HeightsGenerator {    
    ...
    fn get_height(&self, x: isize, z: isize) -> f32 {
        ...
        for _ in 0..HeightsGenerator::OCTAVES {            
            ...
            total_noise += self.get_interpolated_noise(x / octave_width_f32, z / octave_width_f32, octave_width) * amplitude;
